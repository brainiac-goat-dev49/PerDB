...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // Catch-all handler without path-to-regexp syntax to guarantee compatibility with all Express / Node versions
    app.use((req, res) => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`PerDB server listening on http://0.0.0.0:${PORT}`);
  });
}
